...
local Groups = require("cozy.groups")
...
function M.extras()
  local Extra = require("cozy.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/cozy/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/cozy/config.lua", "\n(--@class cozy%.Config.-\n})")
